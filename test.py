...
def test_all_checkpoints(
    ...
    for ckpt_name in ckpt_files:
        ...
        # 禁用梯度计算
        with torch.no_grad():
            auroc_sp, aupr_sp, acc, f1 = evaluation(
                encoder,
                decoder,
                res,
                test_loader,
        ...
        print(f"  AUROC: {auroc_sp:.4f} | AUPR: {aupr_sp:.4f} | ACC: {acc:.4f} | AUPR: {f1:.4f}" )
    ...
    with open(result_path, 'w') as f:
        ...
        for res in all_results:
            ...
            f.write(f"  AUROC: {res['acc']:.4f}\n")
            f.write(f"  AUPR: {res['f1']:.4f}\n")
            f.write("-------------------------------\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--class_name", default="luna_vae_exp1_res6",type=str,  help="Name of the product class")
    parser.add_argument("--ckpt_dir", type=str, default="./checkpoints/luna_vae", help="Path to the directory containing checkpoints")
    parser.add_argument("--data_path", default="/home/data3t/luxijun/document/Dataset/adluna" ,type=str, help="Path to the dataset")
    parser.add_argument("--res", type=int, default=6, help="Resolution parameter for the model")
